...
#define PAIR_CONFIRM_TIMEOUT_MS 60000
#define PAIR_CONFIRM_POLL_MS 1000

static int pair_request_wait_paired(char* url, PHTTP_DATA data, int timeout_ms) {
  int ret;
  int elapsed_ms = 0;
  char* paired = NULL;
  ...
  do {
    ...
    if (elapsed_ms >= timeout_ms) {
      free(paired);
      gs_error = "Timed out waiting for host PIN confirmation";
      return GS_FAILED;
    }

    printf("Waiting for host PIN confirmation...\n");
    fflush(stdout);
    usleep(PAIR_CONFIRM_POLL_MS * 1000);
    elapsed_ms += PAIR_CONFIRM_POLL_MS;
  } while (true);
}
...
int gs_pair(PSERVER_DATA server, char* pin) {
  ...
  if (data == NULL)
    return GS_OUT_OF_MEMORY;
  else if ((ret = pair_request_wait_paired(url, data, PAIR_CONFIRM_TIMEOUT_MS)) != GS_OK)
    goto cleanup;
  ...
  BIO_puts(bio, plaincert);
  X509* serverCert = PEM_read_bio_X509(bio, NULL, NULL, NULL);
  BIO_free(bio);
  if (serverCert == NULL) {
    gs_error = "Error loading server cert into memory";
    ret = GS_FAILED;
    goto cleanup;
  }

  const ASN1_BIT_STRING *serverAsnSignature;
  X509_get0_signature(&serverAsnSignature, NULL, serverCert);
  int serverSignatureLength = ASN1_STRING_length(serverAsnSignature);
  int expectedResponseLength = sizeof(challenge_data) + serverSignatureLength + 16;
  unsigned char *expectedResponse = malloc(expectedResponseLength);
  ...
  memcpy(expectedResponse, challenge_data, sizeof(challenge_data));
  memcpy(expectedResponse + sizeof(challenge_data), ASN1_STRING_get0_data(serverAsnSignature), serverSignatureLength);
  memcpy(expectedResponse + sizeof(challenge_data) + serverSignatureLength, pairing_secret, 16);

  unsigned char expectedResponseHash[32];
  if (server->serverMajorVersion >= 7)
    SHA256(expectedResponse, expectedResponseLength, expectedResponseHash);
  else
    SHA1(expectedResponse, expectedResponseLength, expectedResponseHash);

  free(expectedResponse);
  X509_free(serverCert);

  if (memcmp(expectedResponseHash, challenge_response_data, hash_length) != 0) {
  ...
  server->paired = true;
  printf("Pairing authorization complete\n");
