...
static void parse_argument(int c, char* value, PCONFIGURATION config) {
  ...
  case '4':
    ...
    break;
  case '5':
    if (value != NULL && strlen(value) > 0 && strlen(value) <= 4 &&
        strspn(value, "0123456789") == strlen(value)) {
      snprintf(config->pin, sizeof(config->pin), "%04d", atoi(value));
    }
    break;
  case '6':
...
void config_parse(int argc, char* argv[], PCONFIGURATION config) {
  ...
  if (config->key_dir[0] == 0x0) {
    ...
    else if ((dir = getenv("HOME")) != NULL && strcmp(dir, "/") != 0)
      snprintf(config->key_dir, sizeof(config->key_dir), "%s" DEFAULT_CACHE_DIR MOONLIGHT_PATH, dir);
    else if (access("/userdata", W_OK) == 0)
      snprintf(config->key_dir, sizeof(config->key_dir), "/userdata" DEFAULT_CACHE_DIR MOONLIGHT_PATH);
    else if (access("/userdisk", W_OK) == 0)
      snprintf(config->key_dir, sizeof(config->key_dir), "/userdisk" DEFAULT_CACHE_DIR MOONLIGHT_PATH);
    else if (pw != NULL)
      snprintf(config->key_dir, sizeof(config->key_dir), "%s" DEFAULT_CACHE_DIR MOONLIGHT_PATH, pw->pw_dir);
    else
      snprintf(config->key_dir, sizeof(config->key_dir), "/tmp" MOONLIGHT_PATH);
  }
