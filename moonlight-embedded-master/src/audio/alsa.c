...
static int outputVolumePercent = 100;

static int parse_output_volume_percent(void) {
  const char* value = getenv("MOONLIGHT_RK_BASE_VOLUME");
...
static void apply_output_volume(short* samples, int sampleCount) {
  ...
  for (int i = 0; i < sampleCount; i++)
    samples[i] = (short)((int)samples[i] * outputVolumePercent / 100);
}
...
static int alsa_renderer_init(int audioConfiguration, POPUS_MULTISTREAM_CONFIGURATION opusConfig, void* context, int arFlags) {
  ...
  outputVolumePercent = parse_output_volume_percent();
  printf("Moonlight RK audio baseline volume %d%%\n", outputVolumePercent);
...
static void alsa_renderer_decode_and_play_sample(char* data, int length) {
  ...
  if (decodeLen > 0) {
    apply_output_volume(pcmBuffer, decodeLen * channelCount);
