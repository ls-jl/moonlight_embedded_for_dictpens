...
static void set_rk_target_env(int width, int height) {
  char value[16];

  snprintf(value, sizeof(value), "%d", width);
  setenv("MOONLIGHT_RK_TARGET_WIDTH", value, 1);
  snprintf(value, sizeof(value), "%d", height);
  setenv("MOONLIGHT_RK_TARGET_HEIGHT", value, 1);
}
...
static void stream(PSERVER_DATA server, PCONFIGURATION config, enum platform system) {
  ...
  #endif
  const char* expect_gamepad = getenv("MOONLIGHT_RK_EXPECT_GAMEPAD");
  if (gamepads == 0 && expect_gamepad != NULL && strcmp(expect_gamepad, "0") != 0) {
    gamepads = 1;
    fprintf(stderr, "No gamepad present at launch; reserving one remote controller for hotplug\n");
  }
  int gamepad_mask = 0;
  ...
  if (system == RK) {
    set_rk_target_env(config->stream.width, config->stream.height);
    if ((drFlags & DISPLAY_ROTATE_MASK) == DISPLAY_ROTATE_90 ||
        (drFlags & DISPLAY_ROTATE_MASK) == DISPLAY_ROTATE_270) {
      int target_width = config->stream.width;
      int target_height = config->stream.height;
      config->stream.width = target_height;
      config->stream.height = target_width;
      fprintf(stderr, "RK rotation target %dx%d; requesting stream %dx%d for rotate %d\n",
              target_width, target_height, config->stream.width, config->stream.height,
              config->rotate);
    }
  }
    ...
    if (config.viewonly) {
      ...
      if (IS_EMBEDDED(system)) {
        ...
        if (config.mapping == NULL && mapping_env == NULL) {
          fprintf(stderr, "No gamecontrollerdb.txt found; using built-in Linux evdev fallback mapping.\n");
        }
