...
static int udev_rescan_handle(int fd) {
  uint64_t expirations;
  while (read(fd, &expirations, sizeof(expirations)) == sizeof(expirations));

  if (autoadd)
    evdev_scan_devices(defaultMappings, debug, inputRotate);

  return LOOP_OK;
}

static void udev_start_rescan_timer(void) {
  rescanFd = timerfd_create(CLOCK_MONOTONIC, TFD_NONBLOCK | TFD_CLOEXEC);
  if (rescanFd < 0) {
    perror("timerfd_create input rescan");
    return;
  }
  ...
  if (timerfd_settime(rescanFd, 0, &spec, NULL) < 0) {
    perror("timerfd_settime input rescan");
    close(rescanFd);
    rescanFd = -1;
    return;
  }

  loop_add_fd(rescanFd, &udev_rescan_handle, POLLIN);
}
...
void udev_init(bool autoload, struct mapping* mappings, bool verbose, int rotate) {
  ...
  if (autoload) {
    ...
    evdev_scan_devices(mappings, verbose, rotate);
  }
  ...
  if (autoload)
    udev_start_rescan_timer();
}

void udev_destroy() {
  if (rescanFd >= 0) {
    loop_remove_fd(rescanFd);
    close(rescanFd);
    rescanFd = -1;
  }
